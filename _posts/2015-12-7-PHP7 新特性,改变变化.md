...
#####yield 关键字是php5.5新增的.可以去看下5.5的新features <http://php.net/manual/en/migration55.new-features.php>

具体就是当程序执行到yield的时候返回值,然后在yield位置继续执行.
而如果使用for或者foreach遍历的时候,每个迭代的中间过程,都会有一个中间变量,每个变量都会占用内容空间,而且分配内容,释放内存都会导致程序运行时间边长,如果使用yield的话,那么所有的中间变量都使用一个内存,那么时间,空间都会变小
